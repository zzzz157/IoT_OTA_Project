...
{
	check_mutex();
    xSemaphoreTake(g_esp_mutex, portMAX_DELAY);
	struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;
	Sockaddr remote;
	char cmd[64];
	remote.port=htons(addr_in->sin_port);
	remote.ip=addr_in->sin_addr.s_addr;
	if(sock_esp8266->connect(sock_esp8266,sockfd,&remote)!=0) 
	{
		xSemaphoreGive(g_esp_mutex);
		return -1;
	}
...
int esp8266_init(void* at_device,char* wifi_name,char* wifi_password)
{
	...
	if (at_dev->Send(at_dev, wifi_cmd, strlen(wifi_cmd), "OK", 20000) != Rx_OK)
	{
